...
export default function LoginPage() {
  ...
  async function handlePasswordReset() {
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      setErrors({ email: "Unesite ispravnu email adresu za reset lozinke" });
      return;
    }
    try {
      ...
      setErrors({ form: "Greška pri slanju reset emaila. Pokušaj ponovo." });
    }
  }
    ...
    <main className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
          <h2 className="text-lg font-semibold text-gray-900 mb-6">Prijavi se</h2>

          {/* Return URL notice */}
          {searchParams.get("returnUrl") && (
            <div className="mb-4 p-3 rounded-lg bg-amber-50 border border-amber-200">
              <p className="text-sm text-amber-800">
                Moraš biti prijavljen/a da bi prijavio/la problem.
              </p>
            </div>
          )}
